...
>
> > Get the value of a single key from the query string. Return None if
...
>
> After it's initialised, it will try to guess the port, and create two
> objects: `path` and `query`.
>
> > Initialise the URL as a string. It accepts strings that have no
> > scheme, but they start with `//`.
>
> > Join the URL with a path
>
> > Replace parts of the url with a new one. The parts that can be
> > replaced are:
> >
> > > -   scheme
> > > -   username
